...
| [E-07](#e-07) | Suppression en lot via l'historique (multi-select + filtres dates) | Évol | `docs/57-bulk-delete-trades.md` |
| [E-06](#e-06) | Winrate par combinaison de setups (modale d'analyse avancée) | Évol | `docs/59-setup-combination-analysis.md` |
...
<a id="e-06"></a>
#### E-06. Winrate par combinaison de setups (modale d'analyse avancée) — ✅
...
- **Livré** (2026-05-05) : analyse **ad-hoc** déclenchée par un bouton "Aller plus loin" sur la tuile « Win Rate & R:R par setup » de PerformanceView. La modale `SetupCombinationDialog` permet de sélectionner N setups (multi-select), clique sur Analyser → backend renvoie les stats agrégées des trades portant **tous** les setups choisis (mode AND), **plus une baseline** sur les mêmes filtres globaux sans la contrainte de combinaison. La modale affiche chaque métrique (nb trades, winrate, R:R moyen, P&L) avec un delta coloré vs baseline (`78% (+12pp vs 66%)`).
- **Backend** : endpoint `POST /stats/setup-combination`, méthode `StatsRepository::getStatsForSetupCombination` (JSON_CONTAINS chaînés en AND), `StatsService::getSetupCombinationAnalysis` (validation, ownership setups + comptes, dédup, plafond 20). Enum `SetupCombinationMatchMode::ALL` (réservé pour AND/OR/EXACT plus tard).
- **Pas de persistance** : c'est volontairement ad-hoc — pas de "groupes nommés" à créer/CRUD. Si l'usage décolle, l'évolution naturelle est `SetupCombination` nommée (à backloger).
- **Doc** : `docs/59-setup-combination-analysis.md`.
...
- **Livré** (2026-05-04) :
  - Filtre **plage de dates** (`date_from` / `date_to`) sur la vue Trades, via le composant `DateRangePicker` partagé (presets 7j/30j/mois/trimestre/YTD).
  - **Sélection multiple** : colonne checkbox PrimeVue + "select all" sur la page courante.
  - **Barre d'action rouge** quand au moins un trade sélectionné, bouton "Supprimer la sélection".
  - **Modale de confirmation** avec récap : nb de trades, plage min→max, comptes concernés (uniques), P&L cumulé impacté (signé + coloré).
  - **Backend transactionnel** : `TradeService::deleteBulk` avec PDO transaction + rollBack sur exception. Plafond 500 IDs par appel (anti-DoS). Endpoint `POST /trades/bulk-delete` protégé par auth + subscription.
  - **Layout uniformisé** : 3 filtres (compte, statut, période) + bouton "Nouveau" sur une seule ligne. Pattern propagé à OrdersView (compte + statut).
  - **Doc** : `docs/57-bulk-delete-trades.md`.

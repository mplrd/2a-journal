...
- **Réserve levée le 2026-08-14** : le support FTMO a répondu. En synthèse, laconique : **ils comptent toutes les requêtes, sans distinguer lectures et écritures ni en examiner la raison**. Le dimensionnement retenu était donc le bon — on avait supposé le pire faute de réponse. Rien à changer côté code : le plafond quotidien porte déjà sur le total des requêtes émises, quelle qu'en soit la nature.
  - Conséquence pratique : **l'intervalle de synchronisation est le seul vrai levier**. Neuf requêtes par cycle et par connexion, donc 96 cycles par jour à 15 minutes font 864 requêtes — sous les 2 000, mais toute anomalie qui double un cycle rapproche du seuil (c'est ce qui s'est produit, voir [97](../97-une-seule-synchro-par-tour.md)).
  - Ce qui n'est **pas** compté par notre compteur : le renouvellement OAuth, qui est un appel HTTPS vers `openapi.ctrader.com` et non une requête au serveur de trading. Depuis la [93](../93-course-refresh-token-partage.md) il n'y en a qu'un par passe pour tout le provider. Notre mesure sous-estime donc d'au plus une requête par cycle — à garder en tête si l'on approche un jour du plafond.
...
- **À faire**, par ordre de délai :
  ...
  3. 🟡 **Garde-fous** — deux sur trois livrés le 2026-08-09 :
     ...
     - ✅ **budget quotidien par connexion** → livré le 2026-08-13, migration **041**, doc [95](../95-budget-quotidien-de-requetes-broker.md). Compteur porté par `broker_connections` (`requests_today` + `requests_counted_on`, remise à zéro implicite au changement de jour UTC), plafond réglable depuis le BO admin (`broker_daily_request_budget`, défaut **1500**, `0` = illimité), clic manuel soumis au même plafond. Atteindre le plafond **reporte** la synchro sans la faire échouer : ni ligne `FAILED`, ni `incrementFailures`, ni disjoncteur. Le compteur est alimenté par le contrat générique `getRequestCounts()` : **MetaTrader couvre les mêmes brokers et s'y branchera sans code supplémentaire** le jour où il l'expose. L'« intervalle dérivé » n'a pas été retenu — un plafond qui reporte est plus simple à comprendre et à régler qu'un intervalle qui se recalcule tout seul.
...
- **Statut au 2026-08-13 : clos, et vérifié en réel.** La ligne `sync_request_budget` des logs de l'env de test donne **9 requêtes par cycle et par connexion** — exactement la cible du refactor, mesurée et non plus estimée. À l'intervalle en vigueur ce jour-là, cela fait **648 requêtes/jour et par connexion**, à comparer au seuil FTMO de 2 000. Le dernier garde-fou (budget quotidien) est livré, et le plafond par défaut de 1 500 laisse de la marge sous le seuil tout en restant très au-dessus de la consommation réelle.
  - Reste en suspens, hors de notre main : **la réponse du support FTMO** à la question du 2026-08-07 — comptent-ils les lectures, ou seulement les écritures ? À reporter ici quand elle arrive ; elle ne conditionne plus qu'un éventuel réglage du plafond, plus la livraison.
  - Ce que la mesure ne dit pas : elle a été prise sur l'environnement de **test**, avec deux connexions. Un compte réel chez FTMO n'a pas été re-synchronisé depuis l'incident.

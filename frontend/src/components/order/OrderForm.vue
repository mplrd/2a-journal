...
function handleSave() {
  ...
  data.expires_at = formatExpiresAt(data.expires_at)
  emit('save', data)
}
...
<template>
  ...
  >
    <div class="flex flex-col gap-4">
      <div>
        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ t('orders.account') }} *</label>
        <Select
          v-model="form.account_id"
          :options="accounts.map((a) => ({ label: a.name, value: a.id }))"
          optionLabel="label"
          optionValue="value"
          :placeholder="t('orders.select_account')"
      ...
      <div>
        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ t('orders.expires_at') }}</label>
        <DatePicker v-model="form.expires_at" showTime hourFormat="24" :minDate="new Date()" class="w-full" />
      </div>

...
<template>
  <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
    ...
    <div class="bg-white rounded-lg border border-gray-200 p-4">
      <div class="text-sm text-gray-500 mb-1">{{ t('dashboard.total_trades') }}</div>
      <div class="text-2xl font-bold">{{ overview?.total_trades ?? 0 }}</div>
    </div>

    <!-- Win Rate -->
    <div class="bg-white rounded-lg border border-gray-200 p-4">
      <div class="text-sm text-gray-500 mb-1">{{ t('dashboard.win_rate') }}</div>
    ...
    <!-- Total P&L -->
    <div class="bg-white rounded-lg border border-gray-200 p-4">
      <div class="text-sm text-gray-500 mb-1">{{ t('dashboard.total_pnl') }}</div>
    ...
    <!-- Profit Factor -->
    <div class="bg-white rounded-lg border border-gray-200 p-4">
      <div class="text-sm text-gray-500 mb-1">{{ t('dashboard.profit_factor') }}</div>
      <div class="text-2xl font-bold">{{ formatRatio(overview?.profit_factor) }}</div>
    </div>

    <!-- Avg RR -->
    <div class="bg-white rounded-lg border border-gray-200 p-4">
      <div class="text-sm text-gray-500 mb-1">{{ t('dashboard.avg_rr') }}</div>
      <div class="text-2xl font-bold">{{ formatRatio(overview?.avg_rr) }}</div>
    </div>

    <!-- Best / Worst Trade -->
    <div class="bg-white rounded-lg border border-gray-200 p-4">
      <div class="text-sm text-gray-500 mb-1">{{ t('dashboard.best_worst') }}</div>

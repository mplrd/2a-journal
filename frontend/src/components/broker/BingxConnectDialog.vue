...
async function connect() {
  ...
  error.value = null
  try {
    await brokerSyncService.createBingxConnection(
      props.account.id,
      apiKey.value,
      apiSecret.value,
    )
...
<template>
  <Dialog
    ...
    :header="t('broker.connect_bingx')"
    modal
    class="w-full max-w-lg"
  >
    <div class="space-y-4">
      <p class="text-sm text-gray-500">{{ t('broker.bingx_instructions') }}</p>

      <div>
        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ t('broker.bingx_api_key') }}</label>
        <InputText v-model="apiKey" class="w-full" :placeholder="t('broker.bingx_api_key_placeholder')" />
      </div>

      <div>
        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ t('broker.bingx_api_secret') }}</label>
        <InputText v-model="apiSecret" class="w-full" type="password" :placeholder="t('broker.bingx_api_secret_placeholder')" />
      </div>
